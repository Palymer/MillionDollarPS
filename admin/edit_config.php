...
#########################################################################
# КОНФИГУРАЦИЯ
# Примечание: пожалуйста, не редактируйте этот файл. Отредактируйте конфиг из раздела администратора.
#########################################################################
...
<h3>Конфигурации</h3>
<p>Параметры на этой странице влияют на работу системы пиксельной рекламы.</p>
<p>Примечание: <i>Убедитесь, что config.php имеет права на запись при редактировании этой формы. Вы должны отключить разрешение на запись после редактирования этой формы.</i></p>
<p><b>Подсказка:</b> Ищете где настройки для сетки? Он устанавливается в «Инвентаризации пикселей» -> <a href="inventory.php"> Управление сетками </a>. Нажмите на Edit, чтобы редактировать параметры сетки.</p>
<p>
	<?php
	if ( is_writable( "../config.php" ) ) {
		echo "- config.php доступен для записи.";
	} else {
		echo "- <font color='red'> Примечание: config.php недоступен для записи. Дайте разрешения на запись в config.php, если вы хотите сохранить изменения</font>";
	}
